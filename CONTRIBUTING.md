...
3. Run the install script to verify generation works: `bash scripts/install.sh`
...
The installed skill at `.opencode/skills/finalcode/` is generated by `scripts/install.sh`. Never edit it directly.
...
- Reference specific files when linking: `[source/references/gates.md](source/references/gates.md)` not "the gates documentation"
...
1. **Test the install script** -- run `bash scripts/install.sh` and verify the output
